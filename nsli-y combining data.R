...
nsli_y <- read.csv("data/nsli-y_2016.csv", sep=",",head=TRUE)  
nsli_y_lang <- read.csv("data/nsli-y_2016_language.csv", sep=",",head=TRUE)
...
head(df)
write.csv(df, file = "data/nsli-y_2016_combined.csv")
#combining columns within one csv file
nsli_y_2 <- read.csv("data/nsli-y_2016-2.csv", sep=",",head=TRUE) 
